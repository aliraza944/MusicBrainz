...
async function getData() {
  ...
  switch (params) {
    case "everything":
      data = await fetchAPI(
        `${baseUrl}recording/?query=artistname:${search}ANDrelease:${search}ANDrecording:${search}&dismax=true&fmt=json`
      );
    ...
    case "artist":
      ...
      break;
    case "album":
      url = `${baseUrl}recording/?query=release:${search}&dismax=true&fmt=json`;
      data = await fetchAPI(url);

      break;
    case "track":
      url = `${baseUrl}recording/?query=name:${search}&dismax=true&fmt=json`;
      data = await fetchAPI(url);
